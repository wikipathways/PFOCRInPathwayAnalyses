...
if(error_flag==1){
  result_0<-c(result_0,list(list(gse_id=unique_gse[i],titles="Failed read")))
}else{
  if(length(grep("chipseq",paste(titles,description),ignore.case = TRUE))>0 | length(grep("chip-seq",paste(titles,description),ignore.case = TRUE))>0){
    chipseq=c(chipseq,i)
    next
  ...
  if(length(group_samples_result)>0){
    results<-c(results,list(group_samples_result))
    result_1<-rbind(result_1,c(i,unique_gse[i]))
    
  }else{
    
    result_0<-c(result_0,list(list(gse_id=unique_gse[i],titles=titles)))
  }
}
...
library(openxlsx)
write.xlsx(unlist(results),"sample_groups.xlsx") 
