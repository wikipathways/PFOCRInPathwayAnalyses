...
rsea_results_human_voom_pfocr=apply(as.matrix(GSE_index),1,function(x){run_rSEA3(as.matrix(pvalue_results_human_voom[,x]),rsea_results_human_voom_pfocr,pfocr_list,pfocr_annotation)})
oNsig_pfocr <- sum(rsea_results_human_voom_pfocr[[1]]$Comp.adjP < 0.05, na.rm = TRUE)
print(oNsig_pfocr)
rNsig_pfcor <- t(sapply(1:Nperm, PermuteDatabase, GSE_index, pfocr_list, pfocr_annotation,pvalue_results_human_voom, run_rSEA3))

GSE_index_2_100_perm_res <- list(TDPbound_full=TDPbound_full, oNsig_pfocr=oNsig_pfocr, rNsig_pfcor=rNsig_pfcor)
saveRDS(GSE_index_2_100_perm_res, file=paste0(outdir, "GSE_index_",GSE_index,"_",Nperm,"_perm_result_only_pfocr.rds"))
